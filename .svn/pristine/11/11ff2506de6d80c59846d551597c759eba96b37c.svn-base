...
public class UserCholDao {
	...
	private RowMapper usercholMapper = new RowMapper() {
		public UserChol mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			userchol.setCheckSeq(rs.getInt("checkSeq"));
			return userchol;
		}
	};

	public int addUserChol(final UserChol userchol) {
		String query = "" +
				"INSERT INTO T_NF_USER_CHOLESTEROL " +
				"	( user_id, ldl, cholesterol, "
				+ " tg, hdl, type,"
				+ "status , status2,reg_date,checkSeq) " +
				"VALUES " +
				"	( ?, ?, ?,"
				+ " ?, ?, ?,"
				+ " ?, ?, ?,?) ";
		this.jdbcTemplate.update(query, new Object[] {
			...
			userchol.getStatus2(),
			userchol.getRegDate(),
			userchol.getCheckSeq()
		
		});
		
		query=" select max(seq) from T_NF_USER_CHOLESTEROL where user_id = ? ";
		return this.jdbcTemplate.queryForInt(query, new Object[] {userchol.getUserId()});
	}
	...
	public UserChol getUserCholCheckCheckSeq(int seq) {
		String query = "" + 
				"SELECT * " +
				"FROM T_NF_USER_CHOLESTEROL " +
				"WHERE checkSeq = ? ";
