...
public class QnaDao {
	...
	public List<Qna> getQnaList(String keyword,int category, int page, int itemCountPerPage) {
		String con=" where 1=1 ";
		if(!keyword.equals("")){
			con+=" and (title like '%"+keyword+"%' OR contents_text like '%"+keyword+"%')";
		}
		if(category!=0){
			con+=" and cate_kind =  "+category;
		}
		String query = ""
				+ "    SELECT * FROM ( "
				+ "         SELECT "
				...
				+ "			ORDER BY qna_seq DESC "
				+ "    ) AS a LIMIT " + (page - 1) * itemCountPerPage + "," + itemCountPerPage;
		return (List<Qna>) this.jdbcTemplate.query(query, this.qnaMapper);
	}
	...
	public int getCount(String keyword,int category) {
		...
		String query = " SELECT COUNT(*) FROM T_NF_DOCTOR_QNA "+con;
