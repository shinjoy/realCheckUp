...
public class NoticeDao {
	...
	public List<Notice> getNoticeMainList(String keyword, int page, int itemCountPerPage) {
		...
		if(!keyword.equals("")){
			con+=" and (noti_type = 0 OR noti_type = 1) AND (title like '%"+keyword+"%' OR contents_text like '%"+keyword+"%' )";
		}
		String query = ""
				+ "    SELECT * FROM ( "
				+ "         SELECT "
		...
		return (List<Notice>) this.jdbcTemplate.query(query, this.noticeMapper);
	}
	...
	public int getNoticeMainCount(String keyword) {
		...
		String query = " SELECT COUNT(*) FROM T_NF_DOCTOR_NOTICE "+con;
